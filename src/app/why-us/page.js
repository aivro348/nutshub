  ...
  return (
    <main style={{ minHeight: "100vh", background: "var(--surface-950)", color: "var(--color-text)" }}>
      ...
      <PageHeroHeader
        ...
        subtitle="THE NUTSHUB STANDARD"
        title="The Premium Difference"
        bgImage="/images/hero_products.png"
      />

      {/* COMPARISON TABLE */}
      <section style={{ maxWidth: "950px", margin: "60px auto 80px", padding: "0 20px" }}>
        <h3 style={{ fontSize: "1.9rem", textAlign: "center", marginBottom: "30px", fontFamily: "var(--font-display)", color: "#fff" }}>
            ...
            <tbody>
              {comparisons.map((row, idx) => (
                <tr key={idx} style={{ borderBottom: "1px solid rgba(255,255,255,0.1)" }}>
                  ...
                  <td style={{ padding: "18px", color: "#fff", fontWeight: 600 }}>✨ {row.nutshub}</td>
                  <td style={{ padding: "18px", fontSize: "0.92rem", color: "rgba(255,255,255,0.65)" }}>{row.others}</td>
                </tr>
              ))}
            </tbody>
          </table>
